...
\noindent \textbf{Regularizing priors.} It turns out that the benefits of using regularizing priors can also be explained from a geometrical perspective. Consider the following change to equation (\ref{eq:devil}):
%
...
\noindent \textbf{Non-Centered Parametrization.} Finally, we can change the posterior geometry by using a Non-Centered Parametrization (NCP). One important thing to mention here is that the second solution does not apply indifferently of the problem at hand, in fact, several authors have point out that this 
