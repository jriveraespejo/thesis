...
sim_hit_rate = function(d_true, d_sim, prob=T, hit_rate_fun=hit_rate){
  ...
  var_true = c('IDj','IDk','IDl','IDd','G','A','E','S','Xpu','Xpr','Di')
  ...
  IDmom = IDmom[,c('IDj','true','RMSE',paste0('X',1:S))]
  res = list( IDind = IDmom)
  
  
  # individuals per covariates
  d_mom = data.frame(d_true[c('IDind','GE','AG','ED','SP','XPpu','XPpr','DI')])
  res$IDind = tibble(merge(res$IDind, d_mom, by.x='IDj', by.y='IDind'))
  res$IDind = res$IDind[, c( 1, (S+4):(S+10), 2:3, 4:(S+3) )]
  ...
  mom = mom^2
  IDmom$RMSE = sqrt(rowMeans(mom))
  IDmom = IDmom[,c('IDk','true','RMSE',paste0('X',1:S))]
  res$IDitems = IDmom
  ...
  mom = mom^2
  IDmom$RMSE = sqrt(rowMeans(mom))
  IDmom = IDmom[,c('IDl','true','RMSE',paste0('X',1:S))]
  res$IDtext = IDmom
  ...
  mom = mom^2
  IDmom$RMSE = sqrt(rowMeans(mom))
  IDmom = IDmom[,c('IDd','true','RMSE',paste0('X',1:S))]
  res$IDdim = IDmom
...
#     It plots the statistics of interest 
# arguments:
#     stat_table = object with stats
#     par_int = parameters of interest
#     title = main title for the plot 
#
plot_stat_both = function(stat_table, par_int){

  # # test
  # stat_table
  # par_int = paste0('m_b[',1:5,']') 
  # title='(A)'
  
  # identify parameters
  idx = stat_table$parameter %in% par_int
  
  # plot
  par(mfrow=c(1,2))
  
  # Neff
  idx_var = str_detect(names(stat_table), 'n_eff')
  x_lim = range(stat_table[idx, idx_var])
  plot(stat_table[idx, idx_var], xlim=x_lim, ylim=x_lim,
       col=col.alpha('black',0.5), pch=19, main='(A)',
       xlab='Centered parametrization', ylab='Non-centered parametrization')
  abline(a=0, b=1, lty=2, col=col.alpha('black',0.4))
  abline(v=0, h=0, lty=2, col=col.alpha('black',0.4))
  abline(v=100, h=100, lty=2, col=col.alpha('black',0.4))
  
  # Rhat
  idx_var = str_detect(names(stat_table), 'Rhat4')
  x_lim = range(stat_table[idx, idx_var])
  plot(stat_table[idx, idx_var], xlim=x_lim, ylim=x_lim,
       col=col.alpha('black',0.5), pch=19, main='(B)',
       xlab='Centered parametrization', ylab='Non-centered parametrization')
  abline(a=0, b=1, lty=2, col=col.alpha('black',0.4))
  abline(v=1, h=1, lty=2, col=col.alpha('black',0.4))
  abline(v=1.05, h=1.05, lty=2, col=col.alpha('black',0.4))
...
#     recover_plot
# description:  
#     It plots the parameter's recovery plot 
# arguments:
#     result object = object that contains the statistics of the two 
#     parametrizations in multiple columns
#     par_int = character vector with parameters
#
recover_plot = function(result_object, par_int, title=''){
  ...
  # identify parameters
  idx = which(result_object$parameter %in% par_int)
  
  # reorder correctly
  loc_num = str_locate(result_object$parameter[idx], '[:punct:][:digit:]{1,4}[:punct:]')
  loc_num = as.integer(str_sub(result_object$parameter[idx], start=loc_num[,1]+1, end=loc_num[,2]-1))
  idx = idx[order(loc_num)]
  
  # plot
  y_lim = range( with(result_object[idx,], c(`5.5%.x`, `94.5%.x`,`5.5%.y`,`94.5%.y`)), na.rm=T )
  
  plot( (1:length(idx))-0.1, result_object$mean.x[idx], xlim=c(0, length(idx)+1), ylim=y_lim, 
        xaxt='n', yaxt='n', col=col.alpha('black', 0.3), pch=19, 
        xlab='', ylab='estimates', main=title)
  abline(h=0, col=col.alpha('black', 0.3), lty=2)
  axis(side=1, at=1:length(idx), labels=result_object$parameter[idx], las=2)
  axis(side=2, at=round( seq(y_lim[1], y_lim[2], by=0.1), 2), las=1)
  for(i in 1:length(idx)){
    lines(x=rep(i,2)-0.1, y=with( result_object[idx[i],], c(`5.5%.x`, `94.5%.x`) ) ,
          col=col.alpha('black', 0.3))
  }
  points( (1:length(idx))+0.1, result_object$mean.y[idx], col=col.alpha('blue', 0.3), pch=19)
  for(i in 1:length(idx)){ 
    lines(x=rep(i,2)+0.1, y=with( result_object[idx[i],], c(`5.5%.y`, `94.5%.y`) ) ,
          col=col.alpha('blue', 0.3))
  }
...
#     contrast_stat
# description:  
#     It generates statistics for contrasts 
# arguments:
#     results_object = object of parameters resul
#     post = object with posterior samples
#     contr_pars = character vector of parameter for contrasts
#
contrast_stat = function(results_object, post, contr_pars){
  ...
  for(k in 1:length(contr_pars) ){
    ...
    # selecting parameter
    idx = str_detect(results_object$parameter, contr_pars[k])
    lab_par = results_object$parameter[idx]
    npars = sum(idx)
  ...
  names(diff) = diff_name
  res_diff = precis( diff, depth=4 )
  row.names(res_diff) = NULL
  res_diff = data.frame(parameter=diff_name, res_diff)
