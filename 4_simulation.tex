...
First, to assess the performance of the MCMC chains, in terms of achieving stationarity, convergence and good mixing, the author followed the usual visual approach. The approach involved the visual evaluation of trace plots, for stationarity and convergence (in reality more useful to establish lack of thereof); and trank and autocorrelation plots (ACF), for good mixing. Moreover, the assessment of convergence and mixing were supported by the \textit{potential scale reduction factor} (\texttt{Rhat}) and \textit{effective sample sizes} (\texttt{n\_eff}) statistics developed by \citet{Gelman_et_al_2014} (pp. $284-287$).
...
As stated in previous sections, we analyze two models: (i) the FOLV model depicted in figure \ref{fig:FOLV_model}, and (ii) the SOLV model depicted in figure \ref{fig:SOLV_model}. In this section, we proceed to enumerate the likelihood functions, and the full set of priors and \textit{hyper-priors} used, for the centered and non-centered parametrizations.
...
\noindent where $d$ denotes the index of the individual's dimension. Lastly, because the SOLV contemplates an additional level, corresponding to the reading comprehension ability, an additional set of parameters was defined as follows:
%
\begin{align}
	%
...
\noindent where $\mu^{(2)}_{k}$ and $\sigma^{(2)}_{k}$ are defined as in equations (\ref{eq:mu_items}) and (\ref{eq:sigma_items}), respectively; while $z^{(2)}_{k}$ are samples from independent standard normal distributions, one for each item corresponding to a text passage.

Moreover, equation (\ref{eq:theta_sub}) is re-defined as follows:
%
\begin{align}
	%
...
\noindent where $\boldsymbol{\mu}^{(2)}_{j}$ is defined as in equation (\ref{eq:mu_FOLV}), $\boldsymbol{S}^{(2)}$ is defined as in equation (\ref{eq:sigmas}), and $\mathbf{I}$ is an identity matrix. Additionally, we have:
%
\begin{align}
	%
...
\noindent where $z_{jd}$ are samples from independent standard normal distributions (one for each dimension and individual); while $\boldsymbol{L}^{(2)}_{\Sigma}$ denotes the Cholesky factorization of correlation matrix $\boldsymbol{R}^{(2)}$. A Cholesky factorization is a decomposition of any positive-definite matrix $\mathbf{A}$, into the product of a lower triangular matrix and its conjugate transpose, i.e. $\mathbf{A} = \boldsymbol{L} \cdot \boldsymbol{L}^{T}$. Notice the prior distribution of the decomposition $\text{LKJCorrelationCholesky}(\cdot)$, imply the same assumption as before, that is, $\boldsymbol{R}^{(2)} = \boldsymbol{L}^{(2)}_{\Sigma} \cdot \boldsymbol{L}^{(2)T}_{\Sigma} \sim  \text{LKJCorrelation}(2)$\footnote{See \citet{Stan2020} for a detailed explanation on the implementation.}.

Furthermore, as in the previous cases, equation (\ref{eq:theta}) also shows sampling dependence. Therefore, the equation is re-defined as follows:
%
\begin{align}
	%
...
\noindent where $\mu^{(3)}_{j}$ is defined as in equation (\ref{eq:mu_SOLV}), and $z_{j}$ are samples from independent standard normal distributions, one for each individual. Finally, the remaining equations defined under the CP maintain their description under the NCP.
