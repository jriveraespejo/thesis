...
\noindent where $\text{LkjCorrelation}(\cdot)$ denotes the Lewandowski, Kurowicka, and Joe prior correlation distribution \cite{Lewandowski_et_al_2009}. Notice from equation (\ref{eq:sigma_factoring}), and the aforementioned prior, that the current research departs from the usual setting of prior distributions for covariances matrices. As it is point out by \citet{Depaoli_2021}, the literature on bayesian MSEM/CFA have favored the use of the Inverse-Wishart distribution, based on its conjugancy properties. However, the assumptions derived from its use, that is, the priors are equally informative for variances and covariances, is sometimes restrictive. Following \citet{McElreath_2020}, we believe that by factoring the covariance matrix into variances and correlations, allow us gain control on the preliminary assumptions set to those parameters. Moreover, it makes easier to identify when the researcher is using the unit variance identification (UVI) scheme to ensure, the model's identification.

In that sense, in order to fully specify the model and provide a scale for the latent variables, the current research decided to use the UVI scheme, i.e. set the scale of the higher-order dimension and sub-dimensions to one,  $\sigma^{(3)}_{\theta} = 1$, and $\mathbf{S}^{(2)} = \pmb{\sigma}^{(2)}_{\theta} \mathbf{I}$ with $\pmb{\sigma}^{(2)}_{\theta} = [1, 1, 1]^{T}$. Moreover, we identified the texts' and items' by setting $\mu_{\eta} = 0$, $\sigma_{\eta}=1$, $\delta_{\eta}=2$.
